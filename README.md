...
- `functions/_shared/chat-proxy.js` powers the AI chat page: a same-origin proxy (`/api/chat/proxy`, GET + POST) that forwards requests to any OpenAI-compatible endpoint named in the `X-Target-Url` header, passing through the caller's `Authorization` header and streaming the response back. This avoids CORS issues when the browser talks to third-party model APIs. Forwarding is restricted to an allowlist of hosts (defaults include `token-plan-sgp.xiaomimimo.com` and `api.openai.com`; extend with the `CHAT_PROXY_ALLOWED_HOSTS` env var) so it can't be abused as an open relay. API keys are never stored server-side.
- `src/index.js` is the Cloudflare Worker entry configured by `wrangler.jsonc`; it imports the same shared handlers, adds the KV-backed `GET /api/tweets` endpoint and the `/api/chat/proxy` chat proxy, serves `public/` through the `ASSETS` binding, and runs worker-first API handling for `/api/*`. Because both runtimes import one module, their output is always identical.
